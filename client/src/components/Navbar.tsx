...
const Navbar = () => {
    const user: any = { name: 'John Doe', email: 'email@example.com', isAdmin: true }
    const { cartCount, setIsCartOpen } = { cartCount: 3, setIsCartOpen: (_data: any) => { } }
                                        ...
                                        <div className='absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in'>
                                            {user && (
                                                <div className='px-4 py-2 border-b border-app-border'>
                                                    <p className='text-sm font-medium font-zinc-900'>{user?.name}</p>
                                                    <p className='text-xs font-zinc-900'>{user?.email}</p>
                                                </div>
                                            )}
                                            <div onClick={() => setUserMenuOpen(false)}>
                                                ...
                                                {
                                                    user && <Link to='/products' className='dropdown-link md:hidden'><ArrowUpRightIcon size={16} />Products</Link>
                                                }
                                                {
                                                    user && <Link to='/deals' className='dropdown-link md:hidden'><ArrowUpRightIcon size={16} />Deals</Link>
                                                }
                                                {user?.isAdmin && (
